...
In the first one, the left hand side would stay the same as "n + 0", but in the second one Coq 
can simplify the left hand side as "n". This is because Coq can simplify the second one by 
pattern match on the first argument with O in the `plus` function. But pattern match on 'n' in
the first case would not simplify the expression. 
] *)
