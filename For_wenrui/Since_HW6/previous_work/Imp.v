...
Fixpoint optimize_0plus_b (b:bexp) : bexp := 
 ...
 | BTrue  => BTrue 
 | BFalse => BFalse 
 | BEq m n => BEq (optimize_0plus m) (optimize_0plus n) 
 | BLe m n => BLe (optimize_0plus m) (optimize_0plus n)
 | BNot n => BNot (optimize_0plus_b n)
 | BAnd BFalse m => BFalse  
 | BAnd m n => BAnd (optimize_0plus_b m) (optimize_0plus_b n) end.
...
Proof.
 ...
 try (simpl; repeat (rewrite -> optimize_0plus_sound); rewrite IHb2);
 try (simpl; rewrite IHb; reflexivity).

 Case "BAnd".
 destruct b1;
try (simpl; rewrite <- IHb2; reflexivity);
try (simpl; repeat (rewrite optimize_0plus_sound); rewrite IHb2; reflexivity);
try (reflexivity);
...
Inductive bevalR : bexp -> bool -> Prop := 
  | E_True : BTrue || true
  | E_False : BFalse || false
  | E_Eq : forall (e1 e2: aexp) (n1 n2: nat), 
        (aevalR e1 n1) -> (aevalR e2 n2) -> (BEq e1 e2) || (beq_nat n1 n2)
  | E_Le : forall (e1 e2: aexp) (n1 n2: nat), 
        (aevalR e1 n1) -> (aevalR e2 n2) -> (BLe e1 e2) || (ble_nat n1 n2)
  | E_Not : forall (b1: bexp) (b2: bool), (bevalR b1 b2)  -> (BNot b1) || (negb b2)
  | E_And : forall (e1 e2: bexp) (b1 b2: bool),  
        (bevalR e1 b1) -> (bevalR e2 b2) -> (BAnd e1 e2) || (andb b1 b2)
...
try (simpl; rewrite aeval_iff_aevalR in H; rewrite aeval_iff_aevalR in H0; rewrite H; rewrite H0; reflexivity).

Case "<-". generalize dependent b.  induction e; 
simpl; intros; subst; 
try apply E_True;
try apply E_False;
try apply E_Eq;
...
try apply E_Not;
try apply E_And;
repeat (rewrite aeval_iff_aevalR);
try (apply IHe);
try (apply IHe1);
try (apply IHe2);
 reflexivity.
...
assert (beq_id x x = true). symmetry.
...
Proof.
intros.
unfold update. remember  (beq_id x1 x2) as b.
destruct b;
try apply beq_id_eq in Heqb; subst; reflexivity.
Qed.
...
Proof.
intros. unfold update.
remember (beq_id x1 x3) as b1. 
remember (beq_id x2 x3) as b2.
destruct b1;
destruct b2;
 try (apply beq_id_eq in Heqb1; apply beq_id_eq in Heqb2; subst;
 rewrite <- beq_id_refl in H; inversion H); reflexivity.
...
reflexivity.
apply E_Seq with 
(st' :=  (update (update (update empty_state X 2) Y 0) Y 2));
apply E_Ass; reflexivity.
...
Proof.
  ...
  (* Proceed by induction on the assumed derivation showing that
     ...
     [inversion]). *)
 induction contra; inversion Heqloopdef; try subst; inversion H.
...
Inductive no_whilesR: com -> Prop :=
 ...
 | no_whilesR_ASS : forall A X, no_whilesR (A ::= X)
 | no_whilesR_Seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 
                                                -> no_whilesR (c1;c2)
 | no_whilesR_If : forall c1 c2 b, no_whilesR c1 -> no_whilesR c2
...
Case "->".
intros. induction c;
try apply no_whilesR_SKIP;
try apply no_whilesR_ASS;
try apply no_whilesR_Seq;
simpl in H; try  apply andb_true_iff in H;
inversion H; try apply IHc1 in H0; try apply IHc2 in H1;
try apply H0; try apply H1;
try apply no_whilesR_If; auto.

Case "<-".
intros. induction c;
try reflexivity;
inversion H; try apply IHc1 in H2; try apply IHc2 in H3; try apply IHc2 in H4; simpl;
subst; apply andb_true_intro; split; try apply H2; try apply H3; try apply H4.
Qed.
...
intros. generalize dependent st.
 induction c.
intro. exists st. apply E_Skip.
intro. exists (update st i (aeval st a)). apply E_Ass. reflexivity.
...
apply E_Seq with (c1:= c1) (c2:=c2) (st := st) (st' := x) (st'' := x0). apply H1. apply H5.

intro. 
 inversion H. subst.
 assert (exists st', c1 / st || st'). apply IHc1. apply H2.
 assert (exists st', c2 / st || st'). apply IHc2. apply H4.
  remember (beval st b) as b1. destruct b1.
  SCase "IF_true". inversion H0. exists x. apply E_IfTrue. symmetry.  apply Heqb1. apply H3.
  SCase "IF_false". inversion H1. exists x. apply E_IfFalse. symmetry.  apply Heqb1. apply H3.
...
match stack, prog with 
 | _, [] => stack
 | _, (SPush n)::l_inst =>  s_execute st (n::stack) l_inst 
 | _, (SLoad x)::l_inst => s_execute st ((st x)::stack) l_inst
 | n::m::l_stack, SPlus::l_inst => s_execute st ((m + n)::l_stack) l_inst 
 | n::m::l_stack, SMinus::l_inst => s_execute st ((m - n)::l_stack) l_inst
 | n::m::l_stack, SMult::l_inst => s_execute st ((m * n)::l_stack) l_inst 
 | _, _ => []
end. 
(* TODO unspecified execution how to define *)
...
(*
Lemma s_compile_connect : forall (st: state) (insts1 insts2 : list sinstr),
s_execute st [] (insts1++insts2) = s_execute st (s_execute st [] insts1) insts2.
Proof.
Admitted.
*)
...
(*rewrite s_compile_connect. rewrite IHe1.unfold *)
Admitted.
(** [] *)
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Seq_1 : forall st st' c1 c2, c1 / st || SBreak / st' -> 
                                 (CSeq c1 c2) / st || SBreak / st'
  | E_Seq_2 : forall st st' st'' sig c1 c2, c1 / st || SContinue / st' ->
                                            c2 / st' || sig / st'' ->
                                     (CSeq c1 c2) / st || sig / st''
  | E_If_1 : forall b c1 c2 sig st st', beval st b = true -> 
                             c1 / st || sig / st' ->
                             (CIf b c1 c2) / st || sig / st'
  | E_If_2 : forall b c1 c2 sig st st', beval st b = false ->
                            c2 / st || sig / st' ->
                            (CIf b c1 c2) / st || sig / st'
  | E_While_1 : forall b c st st' st'', beval st b = true -> 
                              c / st || SContinue / st' -> 
                           (CWhile b c) / st' || SContinue / st'' -> 
                           (CWhile b c) / st || SContinue / st''
  | E_While_2 : forall b c st st', beval st b = true ->
                              c / st || SBreak / st' -> 
                              (CWhile b c) / st || SContinue / st'
  | E_While_3 : forall b c st, beval st b = false -> 
                               (CWhile b c) / st || SContinue / st
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  [ Case_aux c "E_Skip" | Case_aux c "E_Break" | Case_aux c "E_Ass" |  
    Case_aux c "E_Seq_1" | Case_aux c "E_Seq_2" | Case_aux c "E_If_1" |
    Case_aux c "E_If_2" | Case_aux c "E_While_1" | Case_aux c "E_While_2" |
    Case_aux c "E_While_3"].
...
ceval_cases (induction H1) Case; intros; inversion H2; subst.

Case "E_Skip". split; reflexivity.
Case "E_Break". split; reflexivity.
Case "E_Ass". split; reflexivity.

Case "E_Seq_1". apply IHceval in H6. apply H6.
                apply IHceval in H3. inversion H3. inversion H0.

Case "E_Seq_2". apply IHceval1 in H5. inversion H5. inversion H0.
                apply IHceval2. apply IHceval1 in H1. inversion H1.
                rewrite H. apply H6. 

Case "E_If_1". apply IHceval. apply H9. rewrite H in H8. inversion H8.
               
Case "E_If_2". rewrite H in H8. inversion H8. apply IHceval. apply H9.

Case "E_While_1". apply IHceval1 in H4.  inversion H4. subst. clear H4.
                  clear H1. clear H3. apply IHceval2. apply H8. 
                  apply IHceval1 in H7.  inversion H7. inversion H1.
                  rewrite H in H6. inversion H6.

Case "E_While_2". apply IHceval in H5. inversion H5. inversion H3.
                  apply IHceval in H8. apply proj1 in H8. split.
                  apply H8. reflexivity. rewrite H7 in H. inversion H.

Case "E_While_3". rewrite H in H3. inversion H3.
