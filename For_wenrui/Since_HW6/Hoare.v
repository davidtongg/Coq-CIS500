...
~({{(fun st => True) [X |-> APlus (AId X) (ANum 1)]}}
(X ::= ANum 2)
{{fun st => (st X = 2)}}
).
(*TODO: Need to prove?*)
...
split.
Case "Q st'". assumption. 
Case "st X = aeval st' a". unfold update. simpl. trivial.

Case "proof for replace".
symmetry. apply functional_extensionality. intro x.
rewrite update_shadow. 
remember (beq_id X x) as b. destruct b.
 SCase "X is x". apply beq_id_eq in Heqb. subst. unfold update. simpl. trivial.
 SCase "X is not x". apply update_neq. symmetry.  assumption.
Qed.
...
intros st st' H1 H2.
destruct H2;
inversion H1; subst; unfold update; simpl; constructor; assumption.
Qed.

Example hoare_asgn_examples2' :
{{fun st => (0 <= 3 /\ 3 <= 5)}}
(X ::= (ANum 3))
{{fun st => (0 <= (aeval st (AId X)) /\ (aeval st (AId X)) <= 5)}}.
Proof.
intros st st' H1 H2.
inversion H1. subst.
...
replace (update (update (update st Z (st X)) X (update st Z (st X) Y)) Y
     (update (update st Z (st X)) X (update st Z (st X) Y) Z) Y) with (st X).
replace (update (update (update st Z (st X)) X (update st Z (st X) Y)) Y
     (update (update st Z (st X)) X (update st Z (st X) Y) Z) X) with (st Y).
assumption.

rewrite update_neq. rewrite update_eq. rewrite update_neq. trivial. reflexivity. reflexivity.
rewrite update_eq. rewrite update_neq. rewrite update_eq. trivial. reflexivity.
Qed.
...
(* Answer:
This proposition might not be true in the first place.
The program has two steps, and the first step assigns value 
to the variable X. This step might potentially alter the arithmetic 
expression a (for the step of aeval st a). Therefore, the precondition
"aeval st a = n" is not an invariant during the execution of the program.
It follows that (st Y = n) is not necessarily true.
